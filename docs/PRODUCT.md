...
- 压缩、归档保护、大小告警、历史保留和调度设置（v1 固定单卷）；
- 可选外部源映射。
...
默认用户入口是备份方案管理与源目录树，不要求用户先定位已有 `config.db`、手写 `.backupignore` 或理解迁移日志。首次使用应能建立本机配置并创建 Managed 方案；选择已有数据库属于高级配置恢复/维护入口。

已确认的默认方式：用户点击“开始使用”后，软件在系统当前用户的应用数据目录（.NET `LocalApplicationData`）下管理 `StowCrate/config.db`，首次创建，后续复用并验证已有库。无法取得该目录或已有库损坏时明确报告，不回退到工作目录，也不删除重建已有库。手动选择已有库保留为高级入口。备份源和归档输出位置仍由用户独立选择；自动创建配置目录不改变迁移目标根必须已存在、缺失时提示用户创建的约束。

主配置界面以目录树显示状态：普通目录与“📦 归档箱”视觉区分。选择归档箱后配置模式、规则、格式、保护、历史和大小警告。规则编辑提供“选择文件”“选择文件夹”“文件类型”“高级规则”四个层级。

新建 Managed 方案的首个界面步骤保存方案名称、首个备份源的逻辑名称和输出相对目录（必填且非空，例如 `projects`），不推断物理路径，也不自动将整个源设为归档箱。输出相对目录遵循 BACKUPPLAN v1 的非空 LogicalPath 约束，不能留空使用输出根，也不从显示名称自动推导。初始配置在保存前明确展示：7z、标准压缩、不加密、历史关闭、手动执行、保留链接对象、Standard 变更检测，三层规则不预设排除项。保存声明后仍须配置本机目录和归档箱，界面必须说明尚不能执行备份；保存成功不等于 readiness 或备份成功。这是分步交付，不能替代下面的完整流程验收。

源目录树的首个增量为只读浏览：从当前方案已保存的源绑定中选择目录，显式读取并展示目录、文件、链接及扫描问题，支持取消。未保存的目录草稿不参与浏览；切换方案、配置库或重新加载绑定会清除旧观察。链接不进入，无法读取或跨文件系统边界的遗漏必须可见。浏览不应用过滤规则，不解析或写入 `.backupignore`，不将显示内容解释成备份选择；归档箱与规则编辑、备份预览仍须独立接入。

执行前提供可审阅计划：将创建、跳过、更新哪些归档，预计大小，被排除内容和配置风险。执行后提供成功、警告、失败、耗时、原始/压缩大小与完整性结果。

首版必须用真实用户流程验收，而不仅是各底层模块测试通过：

1. 新用户从空配置开始添加备份源、选择不重叠的 Current/所需 History 位置，通过目录树将 B、D、F 设为归档箱，不编辑配置文件即可保存方案并重新打开。
2. 在排除/仅包含模式下，以文件、文件夹、文件类型配置规则；预览明确解释单元外文件不备份、父单元不包含子单元，并展示 §4.2 的预期输出结构。
3. 对识别出的开发项目展示建议边界、排除项、文件数与空间估算，确认后应用；不能把 `.git`、`dist` 等内容静默排除。智能配置仍是首版需求，不因底层 Scanner 已完成而算作交付。
4. 手动执行后由第三方工具解压验证；未变化的再次执行跳过重压缩，变化且启用历史时生成独立 History，失败保留有效 Current，取消及遗漏在结果中可见。
5. 用户能配置并查看定时任务的实际安装状态；由 CLI/headless 复用备份用例执行，不能用 Schedule DTO 或数据库字段存在替代运行验收。
6. 用户能获得一致配置快照并按明确说明恢复；普通资料恢复不依赖本软件，Secure 所需独立秘密的恢复要求必须另行说明。

### 5.2 维护入口的边界

存储迁移、恢复日志与释放目录保护属于已有备份的维护能力，不能替代上述默认备份流程。当前开发版本的“存储维护”主窗口是阶段性工作台，不是最终首页设计，也不代表普通用户已能完成第一次备份。现有维护安全协议继续保留；后续界面建设应以补齐默认备份流程为主线，维护功能回归相应入口。

大小告警必须允许继续、调整阈值或关闭提醒；原始需求的 100 MB 用例需要保留，但默认值、MB/MiB 单位及配置归属仍按 §8 待决。不得把告警做成禁止大归档或自动分卷的硬限制，也不得直接向 frozen backupplan v1 添加未经版本契约评审的 WarningPolicy 字段。
...
- `*.backupplan` v1 JSON Schema 已冻结；公开稳定 Schema URI 仍未配置，不得虚构 `$id`；
- 隐私保护恢复信息在 7z、ZIP、TAR.ZST 中的可靠承载方式；
- 7-Zip 密码能否在不出现在命令行或进程列表的前提下可靠地自动传入；
- `config.snapshot.db` 的用户备份位置、版本结构和是否随备份自动产生：原始需求要求配置库快照进入备份目录，而 §4.6 要求 CurrentRoot 不混入数据库。该位置冲突待维护者明确裁决；此前不能把快照写入 Current，也不能把“已有 Online Backup API”当作配置备份用户流程已交付；
