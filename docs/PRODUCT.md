...
默认用户入口是备份方案管理与源目录树，不要求用户先定位已有 `config.db`、手写 `.backupignore` 或理解迁移日志。首次使用应能建立本机配置并创建 Managed 方案；选择已有数据库属于高级配置恢复/维护入口。配置库默认位置及首次创建的交互细节须在实现前落实，不从当前维护页的文件选择器反推默认产品流程。
