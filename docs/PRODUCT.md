...
新建 Managed 方案的首个界面步骤保存方案名称、首个备份源的逻辑名称和输出相对目录（允许为空），不推断物理路径，也不自动将整个源设为归档箱。初始配置在保存前明确展示：7z、标准压缩、不加密、历史关闭、手动执行、保留链接对象、Standard 变更检测，三层规则不预设排除项。保存声明后仍须配置本机目录和归档箱，界面必须说明尚不能执行备份；保存成功不等于 readiness 或备份成功。这是分步交付，不能替代下面的完整流程验收。
