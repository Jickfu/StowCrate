...
- 导入/导出 `.backupignore` 和 `.backupplan`；
- 智能项目识别与建议确认；
- 恢复、验证、配置快照和任务结果查询。
...
1. 解析设备路径映射并验证 Source、Current、History 和 staging 不形成递归关系。
...
Planned
  → Staging (仅外部源/清单等需要时)
  → Writing <name>.<format>.partial
  → Archive Test
  → SHA-256 / manifest verification
  → Move old Current to History (有变化且启用历史)
  → Atomic publish new Current
...
- 在新归档验证成功前，旧 Current 始终有效；
- 若跨卷/跨文件系统移动无法原子完成，先在目标卷生成临时文件再重命名；
...
History 与 Current 的切换需要事务式补偿设计：如果移动旧版本后发布新版本失败，应能恢复旧 Current。
