...
只读检查增量：`StorageRelocationInspectionWorkflow.InspectAsync` 先读取 authoritative configuration 与一致 metadata inventory，调用 `IStorageRelocationInventoryProbe` 验证旧归档 opaque bytes、旧/新根 identity、根内 ancestors、目标占用与容量，最后重新读取配置和 metadata 集合。配置相关漂移、placement/root 集合变化或新增维护互锁均拒绝返回成功；无关名称变化仍允许。物理检查末尾重验整个集合的 namespace/identity，包含零条目的根。预检不创建目录、不签发 durable proof、不构造 journal；结果不代表完整 Preview 可执行。普通 Inspect 不执行完整启动门槛；显式 Begin 另行要求目标真实 case/encoding、namespace 与现存目录 barrier 检查，写入后的真实持久化仍由 Stage/Publish 保证。容量为最近现存目标父目录所在卷的瞬时下界观察，不能替代完整目标布局的 filesystem 检查；也不承诺跨文件原子快照或防御主动 hostile race。
...
显式启动编排：`StorageRelocationBeginWorkflow.BeginAsync` 要求调用方选定 transaction UUID，每次重新执行 inventory、容量、比较规则、namespace 与现存目标目录 barrier 检查；全部物理 I/O 后重验 authoritative configuration 和 metadata 集合，再以 manifest v2 和独立 configuration observation 调用一次原子 Begin。普通 Inspect/InspectTargets 仍只是部分观察，不能缓存为启动授权。预检失败或取消不发出 Begin，不创建目标根或归档临时文件；目标根缺失继续提示用户创建。

Begin 仅返回 PREPARED journal，不自动复制、恢复或 compaction。仓储成功返回后不因 caller cancellation 改报失败。调用已发出后出现 I/O、访问错误、仓储错误或取消，保守报告 RELOCATION_BEGIN_OUTCOME_UNKNOWN 并保留原 transaction UUID；不声称未提交，不自动重试或选取新事务。调用方须读取原 Plan 日志并核对 transaction 后决定显式恢复；损坏日志异常不降级。现存目录预检不保证未来写入成功，Stage/Publish 继续验证真实写入与持久化。App/CLI 装配和其他平台比较能力仍待实施。
