...
5. 当前仓库已完成 portable External/ArchiveSpec/Secret revision readiness、unit-scoped Candidate fingerprints/change decision、ArchiveVersion/placement 与 recoverable journal contract、M3.9 schema/ports、M3.10 EF Core SQLite repository/migration/tests，以及 M3.11 startup discovery/recovery workflow。恢复可在重启后仅凭 filesystem + config.db 对 expected-new Current 自动提交 metadata；observed-old/ambiguous 保留 journal。尚未实现物理发布与 relocation。
