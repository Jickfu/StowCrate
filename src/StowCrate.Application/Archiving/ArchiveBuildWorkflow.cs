...
public sealed class ArchiveBuildWorkflow(
    ...
    IPrivacyRecoveryEnvelopeCodec? recoveryCodec = null)
{
    public async Task<ArchiveBuildResult> BuildAsync(ArchiveBuildRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            ReadOnlyMemory<byte>? recoveryBytes = request.Archive.Candidate.Unit.ArchiveSpec.Protection is PrivacyProtection
                ? recoveryCodec?.Create(request.Archive.Candidate.Unit.ArchiveSpec.Format)
                    ?? throw new InvalidOperationException("Privacy Recovery Envelope codec is unavailable.")
                : null;
            input = await materializer.MaterializeAsync(request, new(manifestBytes, recoveryBytes), cancellationToken).ConfigureAwait(false);
            ...
            var verification = await verifier.VerifyAsync(new(input.Workspace.PartialArtifactPath,
                request.Archive.Candidate.GeneratedMetadata.ManifestPath, request.Archive.Candidate.GeneratedMetadata.RecoveryEnvelopePath, request.Archive.Candidate.Unit.ArchiveSpec,
            ...
            if (request.Archive.Candidate.Unit.ArchiveSpec.Protection is PrivacyProtection)
            {
                if (!verification.RecoveryEnvelopeBytes.HasValue || !verification.RecoveryEnvelopeBytes.Value.Span.SequenceEqual(recoveryBytes.Value.Span))
                    return await CompleteAsync(Failure(ArchiveBuildFailureCode.ManifestMismatch, "Archived Privacy Recovery Envelope does not match generated bytes."), false).ConfigureAwait(false);
            }
